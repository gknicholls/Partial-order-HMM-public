...
showDAGs<-function(B,T,PO,years=1:T) {
  ...
  for (t in years) {
    ...
    showDAG(mr,edge.arrow.size=0.5/length(years),vertex.color=NA,vertex.frame.color=NA,
            vertex.label.cex=1,vertex.size=20,edge.color='black',vertex.label.family="sans")
    text(0,1.2,paste('[',as.character(B+t-1),']',sep=''),col=1,cex=1)
  }
}
...
rZPO<-function(n,K,rho,b=rep(0,n)) {
