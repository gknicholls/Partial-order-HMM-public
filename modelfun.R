# rRprior<-function(n=1,
#                   p=c(0,0.65,0.825,0.925,0.975,0.99,0.9975,0.999,0.9999,0.99999),
#                   w=c(1.25,1.15,1,1.5,1.5,1.65,1.65,1.4,1.4,0.5)) {
#   r=sample(p,n,prob=w,replace=TRUE)
#   return(r)
# }
# 
# dRprior<-function(rho){
#   r=c(0,0.65,0.825,0.925,0.975,0.99,0.9975,0.999,0.9999,0.99999);
#   i=which(r==rho)
...
rRprior<-function(fac=c(1,1/3,8),tol=1e-4) {
  while (1-(rho<-rbeta(1,shape1=fac[1],shape2=fac[2],ncp=fac[3]))<tol) {} 
  return(rho)
}

dRprior<-function(rho,fac=c(1,1/3,8),tol=1e-4) {
  if (rho>1-tol) {return(-Inf)}
  dR=dbeta(rho,fac[1],fac[2],ncp=fac[3],log=TRUE)-pbeta(1-tol,fac[1],fac[2],ncp=fac[3],log=TRUE)
  return(dR)
}
...
ZtoPO<-function(Z,active,B,T,display.Z=FALSE,PO=NA,years=1:T) {
  ...
  for (t in years) {
    ...
    vp<-latent2order(Z[active.t,,t,drop=FALSE]) #GKN 29-08-23 added "drop=FALSE" to make it work when NF=1
...
dPrior<-function(active,U,beta,theta,rho,tau,p,q,cla,B,E,PhPar,dBprior) {
  tot=dTprior(theta)+dRprior(rho,fac=PhPar$rfac)+dBprior(beta)+
