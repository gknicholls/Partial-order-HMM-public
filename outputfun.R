elpd <- function(filename,burnin=1,subsample=1,num.years=T){
  ...
  for(i in 1:nrow(l.list)){
    l.list[i,]=unlist(log.lkd.ser(PO[[i]],cla,year2list(P[i,tau.ind],num.years),years=1:num.years,p=P[i,9],model=note$model))
  }
  ...
  for (i1 in 1:length(PO)){for(i2 in 1:length(PO_bar)){
    PO_bar[[i2]] = PO_bar[[i2]]+PO[[i1]][[i2]]
  }}
  for(i in 1:length(PO_bar)){PO_bar[[i]]=((PO_bar[[i]]/i1)>=0.5)+0 }
  l.bar=sum(unlist(log.lkd.ser(PO_bar,cla,state$y2l,years=1:T,p=mean(P[,"p"]),model=note$model)))
  DIC=2*l.bar-4*mean(P[,1])
  
  #elpd estimation using waic
...
my.load<-function(filename) {
  ...
  output=list(note=note,B=B,E=E,T=T,
              cla=cla,cil=cil,doi=doi,NB=NB,NF=NF,NL=NL,DB=DB,rank=rank,active=active,
              state=state,
              obp=obp,oup=oup,orp=orp,otp=otp,oyp=oyp,oll=oll,oll.tot=oll.tot,
              J=J,start=start,SS=SS,PO=PO,Uout=Uout,P=P,
              db=db,dr=dr,
              step.count=step.count)
  
  if (exists("wide.lists")) output$wide.lists=wide.lists
  if (exists("rng.seed")) output$rng.seed=rng.seed
  if (exists("accept")) {output$accept=accept;output$propose=propose}
  if (exists("st")) output$st=st
  if (exists("opp")) output$opp=opp
  if (exists("oqp")) output$oqp=oqp
  if (exists("drrt")) output$drrt=drrt
  if (exists("PhPar")) output$PhPar=PhPar
  if (exists("MA")) output$MA=MA
  
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) {
    output$Sstate=Sstate
    output$scla=scla
    output$scil=scil
    if (!exists("sNF")) {
      warning('no sNF in synth runfile - DGT - computed from synth setup');
      sNF=dim(Sstate$Z)[2]; 
      if (sNF!=note$sNF) stop('two ways of computing sNF dont match in my.load')
    }
    if (!exists("sNL")) {
      warning('no sNL in synth runfile - DGT - computed from synth setup');
      sNL=length(scla)
    }
    if (!exists("sNB")) {
      warning('no sNB in synth runfile - DGT - computed from synth setup');
      sNB=length(scil)
    }
    if (!exists("sDB")) {
      warning('no sDB in synth runfile - DGT - computed from synth setup');
      srank=ComputeRanks(scil,B,E)
      sDB=max(srank,na.rm=TRUE) 
      if (sDB!=length(Sstate$beta)) stop('two ways of computing sDB dont match in my.load')
    }
    if (!exists("sMA")) {
      warning('no sMA in synth runfile - DGT - computed from synth setup');
      srank=ComputeRanks(scil,B,E)
      sactive=!is.na(srank) 
      sMA=max(apply(sactive,2,sum))
      if (sDB!=length(Sstate$beta)) stop('two ways of computing sDB dont match in my.load')
    }
    output$sNF=sNF; output$sNL=sNL; output$sNB=sNB; output$sDB=sDB; output$sMA=sMA
  }
  
  if (exists("DOPAR",where=note) && note$DOPAR) {
    if (exists("n.blocks")) output$n.blocks=n.blocks
    if (exists("t.blocks")) output$t.blocks=t.blocks
    if (exists("f0")) output$f0=f0
    if (exists("thread.seeds")) output$thread.seeds=thread.seeds
  }
...
my.save<-function(filename,save.list) {
  
  env=as.environment(save.list)
  save(list=names(save.list),file=filename,envir=env)
  
}

dag.conc2<-function(mP,display.threshold=0.5,label.threshold=0.5,fontsize=18,show.closure=TRUE) {
  ...
  e.color <- rep("black",n.edges); names(e.color)<-edgeNames(dag.con)
  e.color[ewv<0.9]<-"black"
  eAttrs$color<-e.color
  e.width <- rep(3,n.edges); names(e.width)<-edgeNames(dag.con)
  e.width[ewv<0.9]<-3
  eAttrs$lwd<-e.width
  e.lty <- rep(1,n.edges); names(e.lty)<-edgeNames(dag.con)
  e.lty[ewv<0.9]<-5
  eAttrs$lty<-e.lty
...
showDAGcon<-function(B,T,PO.con,years=1:T) {
  ...
  for (t in years) {
    ...
    if (numEdges==0) {
      ...
      #if (t==56) {h$nodeAttrs$width[1:19]<-h$nodeAttrs$height[1:19]<-rep("1",19)}
      a=plot(h$dag.con,nodeAttrs=h$nodeAttrs,edgeAttrs=h$edgeAttrs,attrs=h$attrs,"dot")
    }
    text(a@boundBox@upRight@x/2,1*a@boundBox@upRight@y,paste('[',as.character(B+t-1),']',sep=''),col=1,cex=6)
  }
}
...
showDOIs<-function(doi.plot,V.mean,V.sd,cil.plot,B,T,sV=NA,plot.U=TRUE,tau.pm.plot=NA) {
  ...
  for (k in 1:n.doi.plot) {
    ...
    par(mai=c(0,0,0,0))
    if (k!=n.doi.plot && k!=(n.doi.plot-1)) {par(xaxt='n')} else {par(xaxt='s',cex.axis=1.3)}
    if (k%%2==0) {par(yaxt='n')} else {par(yaxt='s',cex.axis=1.3)}
    if (plot.U) {
...
outputanalysis<-function(out.dir,out.file,burn,doi.plot=NA,yoi=1:T,pdf.file=NA,P.samples=NA,full.analysis=TRUE,db=NA,dr=NA,U.con=FALSE) {
  ...
  #out.dir=note$SAVEDIR; out.file=note$savefile; burn=10; yoi=(note$B:note$E)-note$B+1;
  #doi.plot=NA; pdf.file=NA; P.samples=NA; full.analysis=TRUE; db=NA; dr=NA; U.con=FALSE
  
  # out.dir=output.dir; out.file="T-M2-down-sub-cn-os-dir/T-RS5-M2-down-sub-cn-os.RData";
  # burn=100; doi.plot=NA; pdf.file=NA; P.samples=NA; #"paper/T-RS5-M2-down-sub-cn-os"
  # full.analysis=TRUE; db=NA; dr=NA; U.con=FALSE
  
  # out.dir=output.dir; 
  # out.file="H-M2-down-sub-uc-os-synth-b-dir/H-RS4-M2-down-sub-uc-os-synth-b.RData";
  # burn=100; doi.plot=NA; pdf.file="paper/H-RS4-M2-down-sub-uc-os-synth-b";
  # P.samples=NA; full.analysis=TRUE; db=NA; dr=NA; U.con=FALSE;
  ...
  if (exists("MA",where=output)) {MA=output$MA}
  #attach(output)
  ...
  plot(burn:N.sample.done,P[burn:N.sample.done,"obp"],xlab='MCMC-sample',ylab=expression(paste("log ",beta,"-prior",sep='')),type='l'); points(N.sample.done,P[N.sample.done,"obp"],pch=16,col=2)
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) abline(h=Sstate$obp,col=2)
  plot(burn:N.sample.done,P[burn:N.sample.done,"rho"],xlab='MCMC-sample',ylab='',type='l'); points(N.sample.done,P[N.sample.done,"rho"],pch=16,col=2)
  mtext(expression(rho),side=2,line=3,las=1)
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) abline(h=Sstate$rho,col=2)
  plot(burn:N.sample.done,P[burn:N.sample.done,"theta"],xlab='MCMC-sample',ylab='',type='l'); points(N.sample.done,P[N.sample.done,"theta"],pch=16,col=2)
  mtext(expression(theta),side=2,line=3,las=1)
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) abline(h=Sstate$theta,col=2)
  if (exists("DOP",where=note)) {
    plot(burn:N.sample.done,P[burn:N.sample.done,"p"],xlab='MCMC-sample',ylab='',type='l'); points(N.sample.done,P[N.sample.done,"p"],pch=16,col=2)
    mtext("P",side=2,line=3,las=1)
  ...
  if (exists("DOQ",where=note) && note$DOQ) {par(mfrow=c(2,3),mai=0.1*c(8,8,1,1))} else {par(mfrow=c(1,3),mai=0.1*c(8,4,1,1))}
  plot(density(P[burn:N.sample.done,"rho"],from=0,to=1),cex.lab=1.25,xlab=expression(rho),ylab='',main='',ann=TRUE);
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) abline(v=Sstate$rho,col=2)
  par(mai=0.1*c(8,4,1,1))
  plot(density(P[burn:N.sample.done,"theta"],from=0,to=1),cex.lab=1.25,xlab=expression(theta),ylab='',main='',ann=TRUE);
  if (exists("DOSYNTH",where=note) && note$DOSYNTH) abline(v=Sstate$theta,col=2)
  if (exists("DOP",where=note)) {
    par(mai=0.1*c(8,4,1,1))
    plot(density(P[burn:N.sample.done,"p"],from=0,to=1),cex.lab=1.25,xlab='p',ylab='',main='',ann=TRUE); 
  ...
  if (is.na(pdf.file)) {windows()} else {pdf(file=paste(pdf.file,'-beta.pdf',sep=''),paper='a4',6,6)} 
  boxplot(centred.beta[burn:N.sample.done,],xlab='seniority rank', 
          ylab='',cex.lab=1.3,cex.axis=1.5,boxwex=1)
  mtext(expression(paste("effect, ",beta)),cex=1.3,side=2,line=2.5)
  ...
  if (full.analysis) {
    ...
    for (t in years) {
      ...
      PO.con[[t]]=dag.conc2(PO.hat[[t]],display.threshold=db,label.threshold=dr,fontsize=6,show.closure=TRUE) #a list with graph properties
      PO.con.tr[[t]]=dag.conc2(PO.hat[[t]],display.threshold=db,label.threshold=dr,fontsize=6,show.closure=FALSE) #a list with graph properties
      if (U.con) POU.con[[t]]=dag.conc2(POU.hat[[t]],display.threshold=db,label.threshold=dr,fontsize=6,show.closure=TRUE) #a list with graph properties
    ...
    if (FALSE) {
      ...
      dag.con3<-lapply(PO.hat,function(x) my.transitive.closure(x>0.8))
      sgi3=lapply(dag.con3,get.sgi.vsp)
      dag.con4<-lapply(PO.hat,function(x) my.transitive.closure(x>0.5))
    ...
    if (exists("DOSYNTH",where=note) && note$DOSYNTH) {
      POt=lapply(Sstate$h,function(x) {y<-transitive.reduction(x); dag2ig(y,fontsize=5)}) 
      
      if (is.na(pdf.file)) {windows(10,10)} else {pdf(file=paste(pdf.file,'-truePO.pdf',sep=''),6,3)}
      showDAGs(B,T,Sstate$h,years=yoi)
      if (!is.na(pdf.file)) dev.off() 
      ...
      Scoring=lapply(1:T, 
                     function(t) { 
                       Htz=Ht[[t]]; diag(Htz)<-1
                       #ge<-my.transitive.closure(PO.hat[[t]]>=dfn)
                       ge<-(PO.hat[[t]]>=dfn)
                       lt<-!ge
                       an=sum( (Htz==0 & t(Htz)==0 ) )/2 #num pairs of nodes with no reln
                       ap=sum( (Ht[[t]]==1 ) )                   #num of nodes on the > side of a reln
                       fn=sum( (Ht[[t]]==1) & ( lt & t(lt) ) ) #reln in true not in est
                       fp=sum( (Htz==0 & t(Htz)==0) & ( ge | t(ge) ) )/2 #no reln in true but yes in est
                       tp=sum( (Ht[[t]]==1) & (ge)) #reln in both - right way round
                       tn=sum( (Htz==0 & t(Htz)==0) & ( lt & t(lt) ) )/2 #no reln in both
                       wr=sum( (Ht[[t]]==1) & (t(ge) )) #reln in both but wrong way round
                       n.ht=dim(Ht[[t]])[1]
                       if ( 2*(an+ap)!=(n.ht*(n.ht-1))) stop('ap+an issues')
                       #if ( (wr+fn+tp)!= ap) stop('wr+fn+tp issues')
      ...
      if (is.na(pdf.file)) {
        windows()
          ...
          pdf(file=paste(pdf.file,'-false-pos-neg.pdf',sep=''),4,3)
      } 
      dates=years+B-1
      ...
      # # plot(dates,100*(Tp+Tn)/(Tn+Fp+Tp+Fn+Wr),type='l',ylab='% agreeing relations',ylim=c(0,100))
      plot(dates,100*(1-Tp/(Tp+Fn+Wr)),type='l',ylab='% true rejected',ylim=c(0,25),
           cex.axis=0.8,cex.lab=0.8)
      #lines(dates,100*(1-Tn/(Tn+Fp)),type='l',col=3)
            if (!is.na(pdf.file)) dev.off() 
    }
  ...
  if (exists('doi',where=note)) {cnote=note[-unlist(sapply(c('doi','PhPar'),grep,names(note)))]} else {cnote=note}
